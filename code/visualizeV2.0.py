...
def all_plot():
    ...
    sns.pairplot(df_heart, hue='TenYearCHD')
    plt.show()
...
def lable_balance_variable():
    ...
    for x in attributes['nominal']:
        df[x].replace([0,1], ['no', 'yes'], inplace=True)
        ...
        bbox_inches='tight')
        plt.show()
lable_balance_variable()
##################comparing/ visualizing ################################
def corr_plot():
    correlation = df_heart[attributes['continious']].corr(method='pearson')
    fig, ax = plt.subplots(figsize=(10,8))
    plt.title('correlation')
    sns.heatmap(correlation, mask=np.zeros_like(correlation, dtype=np.bool),
                cmap=cm.cividis_r, square=True, ax=ax)
    plt.savefig(os.path.join(figures_path, 'continious variables correlation'))
    plt.show()
...
def violin_plots():
    ...
    if not os.path.exists(path):
        os.mkdir(path)
    for elements in attributes['continious']:
        sns.violinplot(x=df_heart[elements], palette='muted')
